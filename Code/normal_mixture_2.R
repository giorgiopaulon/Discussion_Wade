...
xgrid <- seq(min(X) - 2, max(X) + 2, length.out = 500)

# Inspect the predictive density
ggplot() + 
  geom_histogram(aes(x = X, y = ..density..), bins = 150, fill = 'gray', col = 'white') + 
...
hyperpars <- NULL
hyperpars$m1 <- 0; hyperpars$s1 <- sqrt(100000)
hyperpars$a_sig <- 4; hyperpars$nu1 <- 4; hyperpars$nu2 <- 1
hyperpars$tau1 <- 1; hyperpars$tau2 <- 100
hyperpars$a_alpha <- 2; hyperpars$b_alpha <- 1
...
fit <- blocked_DP_2(X, xgrid, hyperpars, N_tr, Niter, burnin, thin)
...
cols <- c('dodgerblue2','firebrick1','darkorchid1','gold', 'seagreen2')
ggplot() + 
  geom_histogram(aes(x = X, y = ..density..), bins = 100, fill = 'gray', col = 'white') + 
  geom_ribbon(aes(x = xgrid, ymin = pred_quant[1,], ymax = pred_quant[2,]), 
              fill = 1, alpha = 0.3) + 
  geom_line(aes(x = xgrid, y = colMeans(fit$pred)), col = 'black', 
            size = 0.8) + 
  geom_point(aes(x = X, y = colMeans(fit$fit)), col = cols[factor(opt_VI_2)], 
             pch = 1, size = 1.8) + 
  labs(x = 'X', y = 'Density', title = 'Optimal Clustering under VI loss') + 
  scale_color_brewer(name="", palette = "Set1")
...
b2 <- diff(range(fit$sigma))/sqrt(10)
tau1sq <- 1

repuls <- min_repulsive_loss_2D(fit$K, fit$theta, fit$sigma, lambda, c(b1, b2), tau1sq)
...
ggplot() + 
  ...
  geom_point(aes(x = X, y = colMeans(fit$fit)), col = cols[factor(opt_part)], 
             pch = 1, size = 1.8) + 
  labs(x = 'X', y = 'Density', title = 'Optimal Clustering under proposed loss') + 
  scale_color_brewer(name="", palette = "Set1")
